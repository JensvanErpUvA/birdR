...
error_points <- fix_geom(tracks_step2, dt=min_time_inter)
...
```{r NOB}
tracks_step3 <-nob(x=tracks_step3, m=mask_data, unit='hour')
...
track_raster <- rasterizer(data=vect(tracks_step3[,trajectory]),resolution=100)
```

```{r raster_visualization2}
# ## First visual inspection. Convert the data to a data.frame for plotting. Use the spatial mask set in step 2.1.1 to remove "dead" cells
fig_rast_data <- terra::as.data.frame(terra::mask(track_raster,vect(area_of_inclusion)),xy=TRUE)
setnames(fig_rast_data, c("x","y"),c("lon","lat"))
...
tracks_step4 <- distb(x=tracks_step4,track_raster,radar=radar,distance=c(mid,mad))
tracks_final <- tracks_step4[within_raster==TRUE,]
